...
class UpdateParoisseRequest extends FormRequest
{
    ...
    {
        $paroisseId = $this->route('paroisse');
        $contextParoisseId = Auth::check() ? (Auth::user()->paroisse_id ?? null) : null;
        $phoneRegex = (string) ParoisseConfig::get($contextParoisseId, 'phone_regex', '/^(\+242|242|0)?[ \-]?[0-9]{9}$/');
        ...
        return [
            ...
            'code_paroisse' => ['nullable', 'string', 'max:255', Rule::unique('paroisses', 'code_paroisse')->ignore($paroisseId)],
